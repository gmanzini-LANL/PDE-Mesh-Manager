...
public:
  mesh3Dv_printer( mesh_3Dv & _mesh ) : mesh(_mesh) {}
  ~mesh3Dv_printer() {}
...
void mesh3Dv_printer :: print_EdgeVrtx( ostream & LOGF ) {
  ...
  for ( int iE=0; iE<mesh.EdgeVrtx.size(); ++iE ) {
	...
	LOGF << mesh.EdgeVrtx(iE,ilV) << ", " ;
      }
      LOGF << mesh.EdgeVrtx(iE,mesh.EdgeVrtx.size_loc(iE)-1) ;
    }
    LOGF << " } " << endl ;
  }
  LOGF << "-------------------" << endl ;
...
void mesh3Dv_printer :: print_FaceEdge( ostream & LOGF ) {
  ...
  for ( int iF=0; iF<mesh.FaceEdge.size(); ++iF ) {
	...
	LOGF << "(" << mesh.FaceEdge.get_value(iF,j) << ")" << mesh.FaceEdge(iF,j) << ", " ;
      }
...
void mesh3Dv_printer :: print_RegnFace( ostream & LOGF ) {
  ...
  for ( int iR=0; iR<mesh.RegnFace.size(); ++iR ) {
	...
	LOGF << "(" << mesh.RegnFace.get_value(iR,j) << ")" << mesh.RegnFace(iR,j) << ", " ;
      }
...
void mesh3Dv_printer :: print_VrtxEdge( ostream & LOGF ) {
  ...
  for ( int iV=0; iV<mesh.VrtxEdge.size(); ++iV ) {
	...
	LOGF << "(" << mesh.VrtxEdge.get_value(iV,j) << ")" << mesh.VrtxEdge(iV,j) << ", " ;
      }
...
void mesh3Dv_printer :: print_EdgeFace( ostream & LOGF ) {
  ...
  for ( int iE=0; iE<mesh.EdgeFace.size(); ++iE ) {
	...
	LOGF << "(" << mesh.EdgeFace.get_value(iE,j) << ")" << mesh.EdgeFace(iE,j) << ", " ;
      }
...
void mesh3Dv_printer :: print_FaceRegn( ostream & LOGF ) {
  ...
  for ( int iF=0; iF<mesh.FaceRegn.size_loc(iF); ++iF ) {
    LOGF << "iF = " << iF << ", { " 
	 << mesh.FaceRegn(iF,0) << ", " << mesh.FaceRegn(iF,1) ;
  }
  LOGF << " } " << endl ;
  LOGF << "-------------------" << endl ;
}
...
void mesh3Dv_printer :: print_region( int iR, ostream & LOGF ) {
  ...
  for ( int ilF=0; ilF<nRF-1; ++ilF ) {
    LOGF << "(" 
	 ...
	 << mesh.regn_face   (iR,ilF) << ", " ;
  }
  LOGF << "(" 
       << mesh.ok_regn_face(iR,nRF-1) << ")" 
       << mesh.regn_face   (iR,nRF-1) << " }" << endl ;
  LOGF << flush ;
  // topological info
  ...
  LOGF << "---" << endl ;
  LOGF << "#regions:  nRR=" << rlist.size() ; print_list(rlist) ;
  LOGF << "#faces:    nRF=" << flist.size() ; print_list(flist) ;
  LOGF << "#edges:    nRE=" << elist.size() ; print_list(elist) ;
  LOGF << "#vertices: nRV=" << vlist.size() ; print_list(vlist) ;
  // geometrical info
  LOGF << "---" << endl ;
  LOGF << "#center: ( " 
...
void mesh3Dv_printer :: print_face( int iF, ostream & LOGF ) {
  ...
  for ( int ilE=0; ilE<nFE-1; ++ilE ) {
    LOGF << "(" 
	 ...
	 << mesh.face_edge   (iF,ilE) << ", " ;
  }
  LOGF << "(" 
       << mesh.ok_face_edge(iF,nFE-1) << ")" 
       << mesh.face_edge   (iF,nFE-1) << " }" << endl ;
  LOGF << flush ;
  // topological info
  vector<int> rlist, flist, elist, vlist ;
  ...
  LOGF << "---" << endl ;
  LOGF << "#regions:  nFR=" << rlist.size() ; print_list(rlist) ;
  LOGF << "#faces:    nFF=" << flist.size() ; print_list(flist) ;
  LOGF << "#edges:    nFE=" << elist.size() ; print_list(elist) ;
  LOGF << "#vertices: nFV=" << vlist.size() ; print_list(vlist) ;
  // geometrical info
  LOGF << "---" << endl ;
  LOGF << "#center: ( " 
       << mesh.coords_F(iF,0) << ", " 
...
void mesh3Dv_printer :: print_edge( int iE, ostream & LOGF ) {
  LOGF << "--------------------------------------------------------------" << endl ;
  LOGF << "edge: iE =" << iE << endl ;
  // edges
  int nEV = 2 ;
  int iV0 = mesh.edge_vrtx(iE,0) ;
  int iV1 = mesh.edge_vrtx(iE,1) ;
  LOGF << "#vertices: nVE=" << nEV << ", { " ;
  LOGF << mesh.edge_vrtx(iE,0) << ", " 
       << mesh.edge_vrtx(iE,1) << " }" << endl << flush ;
  // topological info
  vector<int> rlist, flist, elist, vlist ;
  ...
  LOGF << "---" << endl ;
  LOGF << "#regions:  nER=" << rlist.size() ; print_list(rlist) ;
  LOGF << "#faces:    nEF=" << flist.size() ; print_list(flist) ;
  LOGF << "#edges:    nEE=" << elist.size() ; print_list(elist) ;
  LOGF << "#vertices: nEV=" << vlist.size() ; print_list(vlist) ;
  // geometrical info
  LOGF << "---" << endl ;
  LOGF << "#center:  ( " 
       << mesh.coords_E(iE,0) << ", " 
  ...
  LOGF << "#vrtx 0:  ( " 
       << mesh.coords_V(iV0,0) << ", " 
       << mesh.coords_V(iV0,1) << ", " 
       << mesh.coords_V(iV0,2) << " )" << endl ; 
  LOGF << "#vrtx 1:  ( " 
       << mesh.coords_V(iV1,0) << ", " 
       << mesh.coords_V(iV1,1) << ", " 
       << mesh.coords_V(iV1,2) << " )" << endl ; 
}
// --------------------------------------------------------------------------------------------
// VERTICES
// --------------------------------------------------------------------------------------------
...
void mesh3Dv_printer :: print_vertex( int iV, ostream & LOGF ) {
  ...
  LOGF << "---" << endl ;
  LOGF << "#regions:  nVR=" << rlist.size() ; print_list(rlist) ;
  LOGF << "#faces:    nVF=" << flist.size() ; print_list(flist) ;
  LOGF << "#edges:    nVE=" << elist.size() ; print_list(elist) ;
  LOGF << "#vertices: nVV=" << vlist.size() ; print_list(vlist) ;
}
